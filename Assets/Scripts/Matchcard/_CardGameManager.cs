...
{   
    public static int sceneMusic = 0;
    public static _CardGameManager Instance;
    public static int gameSize1 = clickS.gameLevel_1;
    public static int gameSize = clickS.gameLevel_2;
    ...
    void Start()
    {
        gameSize1 = clickS.gameLevel_1;
        gameSize = clickS.gameLevel_2;
        StartCardGame();
        gameStart = true;
    ...
    private void SetGamePanel(){
        ...
        int isOdd = (gameSize * gameSize1) % 2 ;

        cards = new _Card[gameSize * gameSize1 - isOdd];
        ...
        float scale = 1.0f/gameSize1;
        float xInc = row_size/gameSize;//left&right
        float yInc = col_size/gameSize1;//up&down
        float curX = -xInc * (float)(gameSize/ 2);
        float curY = -yInc * (float)(gameSize1/ 2);
        Debug.Log("s"+gameSize);//3
        Debug.Log("s1"+gameSize1);//2
        ...
        for (int i = 0; i < gameSize1; i++)
        {
            ...
            for (int j = 0; j < gameSize; j++)
            {
                ...
                {
                    int index = gameSize1 / 2 * gameSize1 + gameSize / 2;
        ...
        if (clickS.levelSelect == 2)
        {
            //rt.transform.localScale = new Vector3(1.08f, 1f, 1f);
            //rt.sizeDelta = new Vector2(200, 200);
        }
        else if (clickS.levelSelect == 1)
        {
            //rt.sizeDelta = new Vector2(300, 300);
        }
        else
        {
            //rt.sizeDelta = new Vector2(450, 400);
            rt.pivot = new Vector2(-0.01f, -0.13f);
            //rt.transform.position = new Vector2(-75f, 25f);
        }
    }
    ...
    private void CheckGameWin()
    {
        ...
        if (cardLeft == 0)
        {
            ...
            EndGame();
            //0SceneManager.LoadScene("Mazes"); 
            //StartCoroutine(CardWin());
        }
    ...
    private void Update(){
        ...
        {
            Gamepanel.transform.position = new Vector3(-75f, 25f, 0f);
        }
    }
}
